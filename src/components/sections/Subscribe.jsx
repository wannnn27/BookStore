    ...
    <section style={{ padding: "0 24px", background: dark ? "#0f172a" : "#f8fafc", paddingBottom: "100px" }}>
      <div style={{
        ...
        <div style={{ position: "relative", zIndex: 1 }}>
          <FadeIn>
            <h2 className="hero-title" style={{ fontSize: 48, fontWeight: 800, color: "#fff", marginBottom: 20, lineHeight: 1.15, letterSpacing: "-1px" }}>
          ...
          <FadeIn delay={0.2}>
            {done ? (
              ...
              <div 
                ...
                style={{ maxWidth: 520, margin: "0 auto", display: "flex", gap: 12, background: "rgba(255,255,255,0.1)", padding: 8, borderRadius: 16, backdropFilter: "blur(10px)", border: "1px solid rgba(255,255,255,0.2)" }}
              >
                <input
                   value={email}
                   onChange={(e) => setEmail(e.target.value)}
                   placeholder="Masukkan alamat email Anda"
                   style={{ flex: 1, padding: "16px 20px", borderRadius: 10, border: "none", fontSize: 16, background: "rgba(255,255,255,0.9)", outline: "none", color: "#0f172a", fontWeight: 500, width: "100%" }}
                />
                <button
                  onClick={() => { if (email && email.includes("@")) setDone(true); }}
                  style={{
                    background: ACCENT, color: "#fff", border: "none", padding: "16px 32px",
